"""
steps/test.py — SageMaker Processing Step: Evaluation

SageMaker mounts:
  /opt/ml/processing/input/test/   → transactions_test.feather
  /opt/ml/processing/input/model/  → TabularTransformer_checkpoint.pth
...
# Keeps all computed values together so evaluate() has a clean return type
# and main() never has to unpack a messy tuple.
...
def main():
    ...
    ckpt_path = os.path.join(args.model_dir,  "TabularTransformer_checkpoint.pth")
    ...
    with mlflow.start_run(run_name=f"evaluation-{job_name}"):
    ...
    # Upload AFTER start_run() closes so the run is fully written to disk
